...
//	int updaterecordstatusSecondlevel(@Param("trainingid")String trainingid,@Param("venueid")String venueid,@Param("additionalbudget")String additionalbudget,@Param("approvedbudget")String approvedbudget);
	...
	int updateTrainingMappingBudgetById(
	  ...
	  @Query("SELECT sv FROM TrainingMappingBudget sv WHERE sv.recordstatus IN ('to be approved', 'waiting for next approval')")
	  List<TrainingMappingBudget> findApprovedAndDelivered();
	  
	  

	  @Query("SELECT sv FROM TrainingMappingBudget sv WHERE sv.recordstatus IN ('to be approved', 'waiting for next approval','delivered','approved')")
	  List<TrainingMappingBudget> findApprovedAndDelivered1();
