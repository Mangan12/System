	...
	<script>
		$(document).ready(function() {
			...
			$(window).on('scroll', function() {
				...
				if (scrollTop > 0) {
		            $filter.css({ 'position': 'sticky', 'top': '4px','z-index': '1000','right':'0px'});
		        }
				else {
		            $filter.css({ 'position': 'fixed', 'top': '20px','z-index': '1000','right':'11px' });
		        }
			});
		});
		...
		function displayTrainingData(data) {
			...
			$('.dataTables_filter').addClass('btn-group').css('font-size', '14px');
		...
		function editTraining(tid, vid) {
			...
			if (isConfirmed) {
				...
				var editUrl = "${pageContext.request.contextPath}/EditViewTrainings.jsp?tid="
						+ tid + "&vid=" + vid; // Redirect to the EditTraining.jsp page with the query string parameters
