			...
			$
					.ajax({
						...
						success : function(data) {
							...
							districtDropdown
									.append('<option value="" selected disabled>Select Resource</option>');

							const uniqueValues = {}; // Store unique values to avoid duplicates

							// Iterate over the data and append only distinct values
							data.forEach(function(item) {
								if (!uniqueValues[item.resources]) {
									districtDropdown.append($('<option>', {
										value : item.resources,
										text : item.resources,
										'data-byear' : item.budgeted_year
									}));
									uniqueValues[item.resources] = true; // Mark value as seen
								}
							});
						},
